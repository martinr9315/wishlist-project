...
app.get('/test', (req, res) => {
  console.log("Got a GET request at my-list.html");
...
function rmfriend(username, listname) {
...
function addfriend(username, listname) {
...
function additem(username, listname, itemname, description) {
  con.query(("INSERT INTO Items VALUES (\""+ mysql.escape(itemname) + "\", \"" + mysql.escape(listname) + "\", \""+ mysql.escape(username) +"\", \""+mysql.escape(description)+"\")"), function (err, result) {
    if (err) throw err;
...
function initDatabase() {
  ...
  con.query("CREATE TABLE Items (ItemName varchar(255), ListName varchar(255), UserName varchar(255), Description varchar(2048));", function (err, result) {
    if (err) throw err;
