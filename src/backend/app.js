...
function getlists(name) {
  con.query(
    'SELECT * FROM Lists WHERE UserName="' + mysql.escape(name) + '"',
    function (err, result) {
      if (err) throw err;
      console.log(result);
    }
...
function getitems(name, listname) {
  con.query(
    'SELECT * FROM Items WHERE ListName="' +
      mysql.escape(listname) +
      '" AND UserName="' +
      mysql.escape(name) +
      '"',
    function (err, result) {
...
function rmuser(name) {
  ...
  con.query(
    'DELETE FROM Users WHERE UserName="' + mysql.escape(name) + '"',
    function (err, result) {
      if (err) throw err;
  ...
  con.query(
    'DELETE FROM Lists WHERE UserName="' + mysql.escape(name) + '"',
    function (err, result) {
      if (err) throw err;
  ...
  con.query(
    'DELETE FROM Items WHERE UserName="' + mysql.escape(name) + '"',
    function (err, result) {
      if (err) throw err;
...
function rmlist(username, listname) {
  ...
  con.query(
    'DELETE FROM Lists WHERE ListName="' +
      mysql.escape(listname) +
      '" AND UserName="' +
  ...
  con.query(
    'DELETE FROM Items WHERE ListName="' +
      mysql.escape(listname) +
      '" AND UserName="' +
      mysql.escape(username) +
      '"',
...
function adduser(name) {
  con.query(
    'INSERT INTO Users VALUES ("' + mysql.escape(name) + '")',
    function (err, result) {
      if (err) throw err;
...
function initDatabase() {
  ...
  con.query(
    "CREATE TABLE Lists (ListName varchar(255), UserName varchar(255));",
    function (err, result) {
      if (err) throw err;
  ...
  con.query(
    "CREATE TABLE Items (ItemName varchar(255), ListName varchar(255), UserName varchar(255));",
    function (err, result) {
      if (err) throw err;
  ...
  con.query(
    "CREATE TABLE Friends (FriendName varchar(255), UserName varchar(255));",
    function (err, result) {
      if (err) throw err;
